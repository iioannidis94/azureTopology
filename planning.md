...
- ✅ **state-management.js** split into 4 modules (`state/` directory):
  ...
  - `js/state/state-cost.js` — dynamic pricing tables, cost calculation, pricing URL
  - `js/state/state-helpers.js` — state query helpers, DNS zone constants

**Status:** `js/state-management.js` re-exports all sub-modules for full backward compatibility.
